              ...
              <tbody className="bg-card divide-y divide-border">
                {groupItems.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.productId}
                      className={`hover:bg-accent ${lowMargin ? 'bg-warning-50' : ''}`}
...
function SortableHeader({
  ...
  return (
    <th
      className={`px-4 py-3 text-${align} text-xs font-medium text-muted-foreground/80 uppercase cursor-pointer hover:bg-accent`}

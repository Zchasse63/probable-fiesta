...
export async function POST(
  ...
  try {
    ...
    // CRITICAL: Create product FIRST before updating deal status
    // This ensures atomic operation - if product creation fails, deal remains pending
    ...
    // CRITICAL: Update deal status with atomic status check to prevent race condition
    // The .eq('status', 'pending') ensures concurrent requests don't both succeed
    ...
    if (updateError || !updatedDeal) {
      // Race condition detected - another request already accepted this deal

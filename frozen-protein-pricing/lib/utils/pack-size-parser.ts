...
/**
 * Asynchronous pack size parser with AI fallback
 * First attempts regex parsing, falls back to AI if regex fails
 * @param packSize - Pack size string to parse
 * @param description - Optional product description for context
 * @returns Parsed weight in pounds or null
 */
export async function parsePackSize(
  ...
  try {
    const response = await fetch('/api/ai/parse-pack-size', {
      ...
      body: JSON.stringify({ packSize, description }),
    });

    if (!response.ok) {
      // AI unavailable or failed, return null
      return null;
    }

    const data = await response.json();
    return data.case_weight_lbs ?? null;
  } catch {
    // AI call failed, return null gracefully
    return null;
  }
}

...
export function sanitizeTextInput(input: string, maxLength: number = 10000): string {
  // Normalize Unicode to ASCII to prevent fullwidth character evasion (e.g., ＼ instead of \)
  let sanitized = input.normalize('NFKC').trim().substring(0, maxLength);
...
export function validateEmailContent(content: string): { valid: boolean; error?: string } {
  ...
  // Check ALL Base64-encoded injection attempts (not just first 3)
  const base64Pattern = /[A-Za-z0-9+/]{20,}={0,2}/g;
  const base64Matches = content.match(base64Pattern);
  if (base64Matches && base64Matches.length > 3) {
    try {
      // Decode and check ALL matches for suspicious patterns

...
async function handlePDFGeneration(priceSheetId: string, isPreview: boolean) {
  try {
    ...
    const products: PDFExportProduct[] = (items || []).map((item: PriceSheetItem) => ({
      ...
      description: (item.products?.description || '').substring(0, 500), // Max 500 chars to prevent DoS
    ...
    if (!isPreview) {
      // Strict filename sanitization: reject if contains '..' or starts with '.'
      // Then whitelist only alphanumeric, dash, underscore
      const zoneName = pdfData.zone_name;
      if (zoneName.includes('..') || zoneName.startsWith('.')) {
        return NextResponse.json(
          { error: 'Invalid zone name - path traversal detected' },
          { status: 400 }
        );
      }
      const sanitizedZone = zoneName

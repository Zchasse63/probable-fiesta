/**
 * Zone Lookup Utility
 * Determines zone from ZIP code using state-to-zone mapping
...
/**
 * State-to-Zone mapping based on zone definitions
 * Zone 1 (Southeast): FL, GA, SC, NC, TN, AL, MS, LA
 * Zone 2 (Northeast): PA, NJ, NY, CT, MA, RI, VT, NH, ME, DE, MD, VA, WV, DC
 * Zone 3 (Midwest): OH, IN, IL, MI, WI, MN, IA, MO, KS, NE, SD, ND
 * Zone 4 (West): CA, OR, WA, NV, AZ, UT, CO, NM, TX, OK, AR
 */
const STATE_TO_ZONE: Record<string, number> = {
  // Zone 1 - Southeast
  FL: 1, GA: 1, SC: 1, NC: 1, TN: 1, AL: 1, MS: 1, LA: 1,
  // Zone 2 - Northeast
  PA: 2, NJ: 2, NY: 2, CT: 2, MA: 2, RI: 2, VT: 2, NH: 2, ME: 2, DE: 2, MD: 2, VA: 2, WV: 2, DC: 2,
  // Zone 3 - Midwest
  OH: 3, IN: 3, IL: 3, MI: 3, WI: 3, MN: 3, IA: 3, MO: 3, KS: 3, NE: 3, SD: 3, ND: 3,
  // Zone 4 - West
  CA: 4, OR: 4, WA: 4, NV: 4, AZ: 4, UT: 4, CO: 4, NM: 4, TX: 4, OK: 4, AR: 4,
};
...
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  '340': 'AA', '341': 'FL', '342': 'FL', '343': 'FL', '344': 'FL', '346': 'FL', '347': 'FL', '349': 'FL',
...
/**
 * Determine zone from ZIP code
 * Returns zone ID or null if cannot determine
 */
export function getZoneFromZip(zip: string): number | null {
  if (!zip) return null;

  // Clean ZIP (remove spaces, hyphens)
  const cleanZip = zip.replace(/[\s-]/g, '');

  // Extract first 3 digits
  const prefix = cleanZip.substring(0, 3);

  // Lookup state from ZIP prefix
  const state = ZIP_PREFIX_TO_STATE[prefix];
  if (!state) {
    // Unknown ZIP prefix - return null silently (expected for invalid ZIPs)
    return null;
  }

  // Lookup zone from state
  const zoneId = STATE_TO_ZONE[state];
  if (!zoneId) {
    // State not mapped - should not happen with complete STATE_TO_ZONE map
    return null;
  }
...
/**
 * Get zone information from ZIP code (async version with DB lookup)
 */
export async function getZoneInfoFromZip(zip: string): Promise<{
  zoneId: number;
  zoneName: string;
  zoneCode: string;
  state: string;
} | null> {
  const zoneId = getZoneFromZip(zip);
